import { mortarTargetBipartiteGraph, Mortar, Target, FO } from "./createDomElements";
...
class FireMission {
    constructor() {
        this.mortars = [];
        this.targets = [];
        this.frontObserver = new FO();
        this.mortarTargetRelationShip = new mortarTargetBipartiteGraph();
        this.htmlElemMort = (() => {
    ...
    calcAllFiringData() {
        ...
        for (var i = 0; i < this.mortarTargetRelationShip.edges.length; i++) {
            ...
            switch (locateMethod) {
                case "Grid":
                    calcMortarFireDataGrid(pair.mortar, pair.target);
                    break;
                case "Polar":
                    calcMortarFireDataPolar(pair.mortar, pair.target, this.frontObserver);
                    break;
                case "Shift (Grid)":
                    calcMortarFireDataShiftGrid(pair.mortar, pair.target);
                    break;
                case "Shift (Polar)":
                    calcMortarFireDataShiftPolar(pair.mortar, pair.target, this.frontObserver);
                    break;
            }
        }
    }
    createLinearTarget() {
        ...
        var attitudeRad = mil2rad(Number(document.getElementById("linTGAtt").value)); // getAttitude();
