...
class GeoLocationElement {
    ...
    //   return this.ObjectType + "_" + this.No.toString();
    // };
    //abstract asignAbstractProperties(prevLoc?:iGeoLocationElement):void;
...
class FO extends GeoLocationElement {
    //DisplayTitle: string = "Front Observer ";
    constructor() {
...
class MTRelationShipInput {
    ...
    updateRelationShips() {
        ...
        for (var k = 0; k < pairableUnits.length; k++) {
            ...
            //option.value = pairableUnits[k].No.toString();
...
//import { getMortarPos, getTargetPos, outPutMortarDistance, outPutMortarDirection } from "./inputOutputFunctions";
// export function calcMortarByGrid(mortarNo:number){
//   var mortarPosition:tPointGrid = getMortarPos(mortarNo);
//   var targetPosition:tPointGrid = getTargetPos(mortarNo);
//   var mortarFireData = getPoint1ToPoint2InPolar(mortarPosition,targetPosition);
//   outPutMortarDistance(mortarNo,mortarFireData.radius);
//   outPutMortarDirection(mortarNo,mortarFireData.radians);
// }
...
function getPoint1ToPoint2InPolar(point1, point2) {
    ...
    //var radiansZtoE = (dEast == 0) ? 0 : Math.atan2(dEast,dNorth);
...
class Map {
    constructor(mortars, targets, frontObserver, MTRelationships) {
        ...
        this.minY = 0;
        this.maxY = 0;
        this.dstncBtwnGridLinesBold = 10; //1000 for production
    ...
    drawVerticalLine(linePos, parentElem, strokeWidth, showLineNum) {
        ...
        //var strokeWidth = isBold ? 0.5 : 0.25;
        lineVGElem.setAttribute("style", 'stroke:rgb(0,255,0);stroke-width:' + strokeWidth);
        parentElem.appendChild(lineVGElem);
        //<line x1="0" y1="0" x2="200" y2="200" style="stroke:rgb(255,0,0);stroke-width:2" />
        if (showLineNum) {
    ...
    setViewBox() {
        ...
        // var sizeLimit = Math.max(maxX-minX,maxY-minY)+dstncBtwnGridLinesBold;
        // var viewBoxValue = 
        // startingX + " " + 
        // (-1)*endingY + " " + 
        // (endingX-startingX) + " " + 
