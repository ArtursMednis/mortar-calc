import { iGeoLocationElement, iMortar_, iTarget_ } from "./basicTypes";
...
constructor(mortars:iMortar_[],targets:iTarget_[],frontObserver:iGeoLocationElement,MTRelationships:iMortarTargetBipartiteGraph) {
...
private addLocationPointOnMap(location:iGeoLocationElement,parentSVG:HTMLElement,color:string|number){
...
private trackMinMax(location:iGeoLocationElement){
  if(Number(location.location.east) == 0 && Number(location.location.north) == 0)
    return;
